// ============================================================
// 交互音效：用 WebAudio 现场合成「咔哒」声，无需音频资源
// - scroll：滚轮每走一格（高频短促，接近 iPod 齿轮声）
// - select：按下中央确认键
// - back：MENU 返回
// iOS 需在首次用户手势中解锁 AudioContext，故提供 unlockSound()
// ============================================================

import { usePrefs } from '@/stores/prefs';

export type TickKind = 'scroll' | 'select' | 'back';

interface Tone {
  freq: number;
  dur: number;
  gain: number;
  type: OscillatorType;
}

const TONES: Record<TickKind, Tone> = {
  scroll: { freq: 2100, dur: 0.012, gain: 0.045, type: 'square' },
  select: { freq: 1180, dur: 0.05, gain: 0.09, type: 'triangle' },
  back: { freq: 640, dur: 0.055, gain: 0.08, type: 'triangle' },
};

let ctx: AudioContext | null = null;

function audioCtx(): AudioContext | null {
  try {
    const Ctor =
      window.AudioContext || (window as unknown as { webkitAudioContext?: typeof AudioContext }).webkitAudioContext;
    if (!Ctor) return null;
    if (!ctx) ctx = new Ctor();
    if (ctx.state === 'suspended') void ctx.resume();
    return ctx;
  } catch {
    return null;
  }
}

/** 在首次用户手势里调用，解锁 iOS WKWebView 的 WebAudio */
export function unlockSound(): void {
  audioCtx();
}

/** 播放一次交互音效；未开启音效时静默返回 */
export function playTick(kind: TickKind = 'select'): void {
  if (!usePrefs.getState().soundFeedback) return;
  const c = audioCtx();
  if (!c) return;

  const t = TONES[kind];
  const now = c.currentTime;
  try {
    const osc = c.createOscillator();
    const gain = c.createGain();
    osc.type = t.type;
    osc.frequency.setValueAtTime(t.freq, now);
    gain.gain.setValueAtTime(0.0001, now);
    gain.gain.linearRampToValueAtTime(t.gain, now + 0.002);
    gain.gain.exponentialRampToValueAtTime(0.0001, now + t.dur);
    osc.connect(gain);
    gain.connect(c.destination);
    osc.start(now);
    osc.stop(now + t.dur + 0.02);
  } catch {
    /* ignore */
  }
}
